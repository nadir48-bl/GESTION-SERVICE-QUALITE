...
for user ,password in zip(userslist,passwordlist):
    try:
        # Connect to MySQL server for the 'data_orge' database
        database = mysql.connector.connect(host="localhost", user=user, password=password)
        cur = database.cursor()

        # Create orgetable if not exists
        cur.execute("CREATE DATABASE IF NOT EXISTS data_orge")
        cur.execute("USE data_orge")
        ...
        cur.execute("""create table if not exists orgetablebulletinsortie
                                       (
                                        num_bulletinor INTEGER AUTO_INCREMENT PRIMARY KEY,
                                        dateor TEXT,
                                        ...
                                        """)
        database.commit()
        # Connect to MySQL server for the 'data_bd' database
        databasebd = mysql.connector.connect(host="localhost", user=user, password=password)
        curbd = databasebd.cursor()

        # Create bdtable if not exists
        curbd.execute("CREATE DATABASE IF NOT EXISTS data_bd")
        curbd.execute("USE data_bd")
        curbd.execute("""CREATE TABLE IF NOT EXISTS bdtable
                       (
                        num_bulletinbd INT AUTO_INCREMENT PRIMARY KEY,
                        datebd TEXT,
        ...
        # Create bdtable_bulletin_sortie if not exists
        curbd.execute("""CREATE TABLE IF NOT EXISTS bdtable_bulletin_sortie
                       (
                        num_bulletinbd INT AUTO_INCREMENT PRIMARY KEY,
                        datebd TEXT,
                        nom_producteurbd TEXT,
        ...
        # Connect to MySQL server for the 'data_bt' database
        databasebt = mysql.connector.connect(host="localhost", user=user, password=password)
        curbt = databasebt.cursor()

        # Create bttable if not exists
        curbt.execute("CREATE DATABASE IF NOT EXISTS data_bt")
        curbt.execute("USE data_bt")
        curbt.execute("""CREATE TABLE IF NOT EXISTS bttable
                       (
                        num_bulletinbt INT AUTO_INCREMENT PRIMARY KEY,
                        datebt TEXT,
        ...
        # Create bttable_bulletin_sortie if not exists
        curbt.execute("""CREATE TABLE IF NOT EXISTS bttable_bulletin_sortie
                       (
                        num_bulletinbts INT AUTO_INCREMENT PRIMARY KEY,
                        datebts TEXT,
    ...
    class Agréage_Window(object):
        ...
        def bulletin_orge_print(self):
            try:
                ...
                self.docorge = DocxTemplate("Docxfiles/bulletin agreage/bulletin_orge/bulletin_orge.docx")
                context = {
                    "nmp": nomproducteur,
                    "per": pere,
                    "mat": matricul,
                ...
                data = mysql.connector.connect(host="localhost", user=user, password=password,database="data_orge")
                cursbd = data.cursor()
                cursbd.execute(
                    "SELECT * FROM orgetablebulletinsortie WHERE num_bulletinor AND dateor=%s AND nom_producteuror = %s AND pereor = %s  AND quantiteor=%s",
                ...
                if existing_data:
                    ...
                    self.docorge.save("Docxfiles/bulletin agreage/bulletin_orge/Orge/" + nameorge)
                    pathorge = os.path.abspath("Docxfiles/bulletin agreage/bulletin_orge/Orge/" + nameorge)
                    databasebd = mysql.connector.connect(host="localhost", user=user, password=password,database="data_orge")
                    curs = databasebd.cursor()
                    curs.execute(
                        "INSERT INTO orgetablebulletinsortie (dateor, nom_producteuror, pereor,quantiteor, pathor,date_sortieor) VALUES (%s,%s,%s, %s, %s, %s)",
        ...
        def add_datta_orge(self):
            try:
                ...
                docorge = DocxTemplate("Docxfiles/bulletin agreage/bulletin_orge/bulletin_orge.docx")
                context = {
                    "nmp": nomproducteur,
                    "per": pere,
                    "mat": matricul,
                ...
                docorge.save("Docxfiles/bulletin agreage/bulletin_orge/Orge/" + nameorge)
                pathorge = os.path.abspath("Docxfiles/bulletin agreage/bulletin_orge/Orge/" + nameorge)
                database = mysql.connector.connect(host="localhost", user=user, password=password,database="data_orge")
                curs = database.cursor()

                # Check if the data already exists
                curs.execute(
                    "SELECT * FROM orgetable WHERE date=%s AND nom_producteur=%s AND pere=%s AND matricule=%s AND point_collect=%s AND nom_agreeur=%s AND quantite=%s AND ps=%s AND total=%s AND path=%s",
                ...
                if existing_data:
                    ...
                    curs.execute(
                        "INSERT INTO orgetable (date ,nom_producteur ,pere ,matricule ,point_collect ,nom_agreeur ,quantite ,ps ,total,path )  values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            ...
            try:
                ...
                if dialog.clickedButton() == ok_button:
                    ...
                    if self.dateor.text() == self.selected_date:
                        ...
                        conn = mysql.connector.connect(host="localhost", user=user, password=password,database="data_orge")
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT num_bulletin, date, nom_producteur, pere, matricule, point_collect,quantite FROM orgetable WHERE DATE(substr(date, 7, 4) || '-' || substr(date, 4, 2) || '-' || substr(date, 1, 2)) =%s;",
                            (selected_date,))
                        ...
                        heading1 = self.doc.add_heading("\t\t\t\t\t\t ÉTAT DES ENTRÉES DE L'ORGE ", level=1)
                        ...
                        hdr_cells[3].width = Inches(2)
                        hdr_cells[4].width = Inches(1.5)
                        hdr_cells[5].width = Inches(2)
                        hdr_cells[6].width = Inches(1)
                        hdr_cells[7].width = Inches(2.5)
                        hdr_cells[8].width = Inches(2.5)
            ...
            if dialogor.clickedButton() == ok_button:
                ...
                try:
                    ...
                    if existing_data:
                        ...
                        curs = data.cursor()
                        curs.execute(
                            "SELECT path FROM orgetable WHERE date=%s AND nom_producteur=%s AND pere=%s AND quantite=%s",
                            (dateentré, producteur, pere, quantite,))
            ...
            QComboBox
            {
               background-color: #ffffff;
                ...
                if dialogor.clickedButton() == ok_button:
                    ...
                    eticketorge = DocxTemplate("Docxfiles/eticket/Eticket_orge/Eticket_orge.docx")
                    eticketorge.render(
        ...
        def bulletin_bd(self):
            try:
                ...
                # Format the date as "dd-mm-yyyy"
                current_days = f"{current_day:02d}-{current_month:02d}-{current_year}-{current_hour}-{current_minute}"
                self.docbd = DocxTemplate("Docxfiles/bulletin agreage/bulletin_blédur/bulletin_blédur.docx")
                self.docbd.render({
                    "nmp": nomproducteurbd,
                    "per": perebd,
                    "mat": matriculbd,
                ...
                datadb = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bd")
                cursbd = datadb.cursor()
                cursbd.execute(
                    "SELECT * FROM bdtable_bulletin_sortie WHERE num_bulletinbd AND datebd=%s AND nom_producteurbd = %s AND perebd = %s  AND quantite=%s",
                ...
                if existing_data:
                    ...
                    try:
                        ...
                        print(e)
                    tempfilebd = nomproducteurbd + "-" + current_days + "B.D" + ".docx"
                    self.docbd.save("Docxfiles/bulletin agreage/bulletin_blédur/BléDur/" + tempfilebd)
                    pathbd = os.path.abspath("Docxfiles/bulletin agreage/bulletin_blédur/BléDur/" + tempfilebd)
                    databasebd = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bd")
                    curs = databasebd.cursor()
                    curs.execute(
                        "INSERT INTO bdtable_bulletin_sortie (datebd, nom_producteurbd, perebd,quantite, pathbd,date_sortie) VALUES (%s,%s,%s, %s, %s, %s)",
        ...
        def add_datta_bd(self):
            try:
                ...
                cursbd = datadb.cursor()
                cursbd.execute(
                    "SELECT * FROM bdtable WHERE datebd=%s AND nom_producteurbd=%s AND perebd=%s AND matriculebd=%s AND point_collectbd=%s AND nom_agreeurbd=%s AND quantitebd=%s AND psbd=%s AND totalbd=%s AND pathbd=%s ",
                ...
                if existing_data:
                    ...
                    cursbd.execute(
                        "INSERT INTO bdtable (datebd ,nom_producteurbd ,perebd ,matriculebd ,point_collectbd ,nom_agreeurbd ,quantitebd ,psbd ,totalbd,pathbd  )  values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                        (datebd, nomproducteurbd, perebd, matriculbd, poindecollectebd, agreeurbd, quantitebd, vpsbd,
                         vtotalbdp, pathbd,))
                    datadb.commit()
                    datadb.close()
        ...
        def print_day_bd(self):
            try:
                ...
                self.datebd.setDisplayFormat("dd-MM-yyyy")
                ...
                if dialog.clickedButton() == ok_button:
                    ...
                    if self.datebd.text() == self.selected_datebd:
                        ...
                        conn = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bd")
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT num_bulletinbd,datebd ,nom_producteurbd ,perebd ,matriculebd ,point_collectbd  ,quantitebd  FROM bdtable WHERE DATE(substr(datebd, 7, 4) || '-' || substr(datebd, 4, 2) || '-' || substr(datebd, 1, 2))=%s;",
                        ...
                        section.right_margin = docx.shared.Cm(1.5)
                        section.left_margin = docx.shared.Cm(1.5)
                        heading1 = self.doc.add_heading("\t\t\t\t\t\t SITUATION DES ENTRÉES DE BLÉ TENDRE ", level=1)
                        heading1.style.font.name = 'Times New Roman'
                        heading1.style.font.size = Pt(22)
                        heading1.style.font.bold = True
                        heading1.style.font.color.rgb = RGBColor(0, 0, 0)
        ...
        def printBulltinProducteur(self):
            ...
            if dialogor.clickedButton() == ok_button:
                ...
                try:
                    ...
                    cursbd = datadb.cursor()
                    cursbd.execute(
                        "SELECT * FROM bdtable_bulletin_sortie WHERE datebd=%s AND nom_producteurbd=%s AND perebd=%s AND quantite=%s",
                        (dateentré, producteur, pere, quantite))
                    existing_data = cursbd.fetchone()
                    if existing_data:
                        msgbox = QtWidgets.QMessageBox()
                        ...
                        try:
                            curs = datadb.cursor()
                            curs.execute(
                                "SELECT pathbd FROM bdtable WHERE datebd=%s AND  nom_producteurbd=%s AND perebd=%s AND quantitebd=%s ",
                                (dateentré, producteur, pere, quantite,)
                            )
                            result = curs.fetchall()
                            print("Result:", result)
            ...
            QComboBox
            {
               background-color: #ffffff;
                ...
                if dialogor.clickedButton() == ok_button:
                    ...
                    eticketorge = DocxTemplate("Docxfiles/eticket/Eticket_BleDur/Eticket_BleDur.docx")
                    eticketorge.render(
                        {"dt": current_date, "ptt": pointdecolect, "nmp": producteur, "mat": mator, "qtt": quantite,
                         "ps": psor, "per": pere})
        ...
        def bulletin_bt(self):
            try:
                ...
                # Format the date as "dd-mm-yyyy"
                current_days = f"{current_day:02d}-{current_month:02d}-{current_year}-{current_hour}-{current_minute}"
                self.docbt = DocxTemplate("Docxfiles/bulletin agreage/bulletin_blétendre/bulletin_blétendre.docx")
                self.docbt.render({
                    "nmp": nomproducteurbt,
                    "per": perebt,
                    "mat": matriculbt,
                    ...
                    "oo": observationbt,
                    "nm": nbulltinbt, })
                databt = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bt")
                cursbt = databt.cursor()
                cursbt.execute(
                    "SELECT * FROM bttable_bulletin_sortie WHERE num_bulletinbts AND datebts=%s AND nom_producteurbts = %s AND perebts = %s  AND quantites=%s",
                ...
                if existing_data:
                    ...
                    try:
                        ...
                        print(e)
                    tempfilebt = nomproducteurbt + "-" + current_days + "B.T" + ".docx"
                    self.docbt.save("Docxfiles/bulletin agreage/bulletin_blétendre/BléTendre/" + tempfilebt)
                    pathbt = os.path.abspath("Docxfiles/bulletin agreage/bulletin_blétendre/BléTendre/" + tempfilebt)
                    databasebt = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bt")
                    curs = databasebt.cursor()
                    curs.execute(
                        "INSERT INTO bttable_bulletin_sortie (datebts, nom_producteurbts, perebts,quantites, pathbts,date_sortie) VALUES (%s,%s,%s, %s, %s, %s)",
        ...
        def add_datta_bt(self):
            try:
                ...
                pathbt = os.path.abspath("Docxfiles/bulletin agreage/bulletin_blétendre/BléTendre/" + tempfilebt)
                # Check if the data already exists
                datadb = mysql.connector.connect(host="localhost", user=user, password=password,database="data_bt")
                cursbt = datadb.cursor()
                cursbt.execute(
                    "SELECT * FROM bttable WHERE datebt=%s AND nom_producteurbt=%s AND perebt=%s AND matriculebt=%s AND point_collectbt=%s AND nom_agreeurbt=%s AND quantitebt=%s AND psbt=%s AND totalbt=%s AND pathbt=%s ",
                ...
                if existing_data:
                    ...
                    cursbt.execute(
                        "INSERT INTO bttable (datebt ,nom_producteurbt ,perebt ,matriculebt ,point_collectbt ,nom_agreeurbt ,quantitebt ,psbt ,totalbt,pathbt  )  values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                        (datebt, nomproducteurbt, perebt, matriculbt, poindecollectebt, agreeurbt, quantitebt, vpsbt,
                         vtotalbtp, pathbt,))
                    datadb.commit()
                    datadb.close()
        ...
        def today_bt(self):
            try:
                ...
                self.datebt.setDisplayFormat("dd-MM-yyyy")
                ...
                if dialog.clickedButton() == ok_button:
                    ...
                    if self.datebt.text() == self.selected_datebt:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT num_bulletinbt, datebt, nom_producteurbt, perebt, matriculebt, point_collectbt, quantitebt FROM bttable WHERE DATE(substr(datebt, 7, 4) || '-' || substr(datebt, 4, 2) || '-' || substr(datebt, 1, 2)) =%s;",
                        ...
                        section.right_margin = docx.shared.Cm(1.5)
                        section.left_margin = docx.shared.Cm(1.5)
                        heading1 = self.doc.add_heading("\t\t\t\t\t\t SITUATION DES ENTRÉES DE BLÉ DUR", level=1)
                        heading1.style.font.name = 'Times New Roman'
                        heading1.style.font.size = Pt(22)
                        heading1.style.font.bold = True
                        heading1.style.font.color.rgb = RGBColor(0, 0, 0)
            ...
            try:
                ...
                if dialog.clickedButton() == ok_button:
                    ...
                    if existing_data:
                        ...
                        curs = datadb.cursor()
                        curs.execute(
                            "SELECT pathbt FROM bttable WHERE datebt=%s AND nom_producteurbt=%s AND perebt=%s AND quantitebt=%s",
                            (dateentré, producteur, pere, quantite))
                        result = curs.fetchall()
                        if result:
                            # Iterate over the paths and open each file
            ...
            QComboBox
            {
               background-color: #ffffff;
                ...
                if dialogor.clickedButton() == ok_button:
                    ...
                    eticketorge = DocxTemplate("Docxfiles/eticket/Eticket_BleTendre/Eticket_BleTendre.docx")
                    eticketorge.render(
                        {"dt": current_date, "ptt": pointdecolect, "nmp": producteur, "mat": mator, "qtt": quantite,
                         "ps": psor, "per": pere})
                    name = f"B.D_{producteur}_{current_daya}.docx"
...
QComboBox::drop-down::icon {
    ...
    QDateTimeEdit::drop-down
    {
        ...
        border-bottom-right-radius: 3px;
            ...
            self.n_bultinbdtxt.resize(120,25)
            ...
            self.nome_du_producteurbdtxt.resize(120,25)
            ...
            self.péretxtbd.setGeometry(QtCore.QRect(315, 98, 100, 25))
            ...
            self.n_cartebdtxt.setGeometry(QtCore.QRect(315, 138, 100, 25))
            ...
            self.imatriculebdtxt = QtWidgets.QLabel("Immatricule:", self.tabbldur)
            self.imatriculebdtxt.move(20, 140)
            self.imatriculebdtxt.resize(120,25)
            ...
            self.pointdecollectbdtxt.resize(120,25)
            ...
            self.dattereceptionbdtxt.setGeometry(QtCore.QRect(880, 100, 150, 25))
            ...
            self.agréeeurbdtxt.setGeometry(QtCore.QRect(880, 135, 150, 25))
            ...
            self.quantitebdtxt.setGeometry(QtCore.QRect(880, 180, 150, 25))
            ...
            self.ergotbd.setStyleSheet("background-color: #232629;color:#00000;")
...
QComboBox::drop-down::icon {
    ...
    QDateEdit::drop-down
    {
        ...
        border-bottom-right-radius: 3px;
            ...
            self.nome_du_producteurbt.resize(150, 30)
            ...
            self.pérebt.setGeometry(QtCore.QRect(410, 98, 150, 30))
            ...
            self.n_cartebt.setGeometry(QtCore.QRect(410, 138, 150, 30))
            self.n_cartebt.setInputMask('99999999')

            self.imatriculebttxt = QtWidgets.QLabel("Immatricule:", self.tabbltendre)
            ...
            self.pointdecollectbt.resize(150, 30)
            ...
            self.dattereceptiontbt.setGeometry(QtCore.QRect(1000, 100, 150, 30))
            ...
            self.agréeeurcombobt.resize(150, 30)
            ...
            self.quantitebt.setGeometry(QtCore.QRect(1000, 178, 150, 30))
...
QComboBox::drop-down::icon {
    ...
    QDateEdit::drop-down
    {
        ...
        border-bottom-right-radius: 3px;
            ...
            self.nome_du_producteur.resize(150, 30)

            self.péretxt = QtWidgets.QLabel("Pére:", self.taborge)
            self.péretxt.setGeometry(QtCore.QRect(315, 98, 100, 25))
            self.pére = QtWidgets.QLineEdit(self.taborge)
            self.pére.setGeometry(QtCore.QRect(410, 98, 150, 30))

            self.n_cartetxt = QtWidgets.QLabel("N.C d'identité:", self.taborge)
            self.n_cartetxt.setGeometry(QtCore.QRect(315, 138, 100, 25))
            self.n_carte = QtWidgets.QLineEdit(self.taborge)
            self.n_carte.setGeometry(QtCore.QRect(410, 138, 150, 30))
            self.n_carte.setInputMask('99999999')

            self.imatriculetxt = QtWidgets.QLabel("Immatricule:", self.taborge)
            ...
            self.pointdecollect.resize(150, 30)
            ...
            self.agréeeurcomboorge.resize(150, 30)

            self.quantitetxt = QtWidgets.QLabel("Quantité", self.taborge)
            self.quantitetxt.setGeometry(QtCore.QRect(880, 180, 150, 25))
            ...
            self.quantiteorge.setGeometry(QtCore.QRect(1000, 178, 150, 30))
